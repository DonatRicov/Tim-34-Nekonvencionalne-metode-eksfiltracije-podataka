...
<p align="center">
<img width="281" height="200" alt="image" src="https://github.com/user-attachments/assets/fae169b0-2024-4e19-8491-9b38464032e0" /></p>
<p align="center"><em>Slika 8: Trokut kompromisa</em></p>
...
4. Napravi izmjene najmanje značajnih bitovatako da se pri njihovoj izolaciji i ponovnom sastavljanu ispiše neka poruka. U slici prikazanoj ispod promjenjeni su bitovi kako bi ispisali slovo e.Slovo, tj. poruka je skrivena u takozvanoj stego slici
 <p align="center">  
<img width="940" height="314" alt="image" src="https://github.com/user-attachments/assets/8d664675-eb0d-48ae-8dfe-234185cdf363" /></p>
<p align="center"><em>Slika 9: Poruka sakrivena u stego slici</em></p>
...
Richard Eason i Eiji Kawaguchi su u radu Principles and Applications of BPCS-Steganography prikazali na koji način se kapacitet skrivanja podataka može povećati čak na 60%. Kako bi unaprijedili tehniku najmanje značajnog bita (LSB), Eason i Kawaguchi iskoristili su ograničenja ljudskog vida. Radi postizanja maksimalne perceptivne neprimjetnosti osmislili su metodu dijeljenja slike na dva različita područja, informativno područje i područje šuma. Informativno područje čini jednostavan uzorak koji oko može trenutačno prepoznati, a područje šuma čini toliko složen uzorak da ga mozak zapravo ne registrira u potpunosti.

Dobar primjer područja šuma bio bi pijesak. Sitna zrna u ogromnim količinama su prvelika količina skoro jednakih objekata da bi ih ljudski mozak mogao razlikovati. Informativno područje bila bi ljubičasta lopta na pjesku koja se jasno istiće u okolini. Na mikroskopskoj razini područja informacija i šuma mogu se matematički mapirati.
<p align="center">
<img width="940" height="309" alt="image" src="https://github.com/user-attachments/assets/9e3ed45a-0376-4d5c-903f-b48ee1261be1" /></p>
<p align="center"><em>Slika 10: Primjeri  područja šuma i informativnog područja</em></p>

Steganografi dijele sliku na 8 razina kako bi mapirali ove regije. 
<p align="center">
<img width="940" height="601" alt="image" src="https://github.com/user-attachments/assets/1fa2ff71-1d30-45ae-95bc-3155afe17d32" /></p>
<p align="center"><em>Slika 11: Podjela na razine</em></p>
...
<p align="center">
<img width="940" height="295" alt="image" src="https://github.com/user-attachments/assets/9057f46f-7c32-4db9-ad2e-b713b855d574" /></p>
<p align="center"><em>Slika 12: Usklađivanje s uzorcima slike</em></p>
...
U praktičnom dijelu rada demonstrirane su različite metode digitalne steganografije korištenjem alata Stegano u Linux terminalu. U odabrane slike umetnute su tri vrste sadržaja: kratka tekstualna poruka, tekstualna datoteka te zvučni zapis. Također su razvijene dvije jednostavne skripte za automatizaciju procesa umetanja i izdvajanja skrivenih podataka iz izmijenjenih slika. Time je omogućeno sustavno testiranje funkcionalnosti steganografskih tehnika te analiza njihovih ograničenja ovisno o formatu slike.

### 3.3.1. Umetanje poruke u terminalu

Za početno testiranje korištena je fotografija snimljena povodom dodjele diploma. Izvorna slika prikazana je na Slikama 1 i 2, gdje je prva spremljena u .jpg, a druga u .png formatu. Ova razlika u formatima omogućila je evaluaciju ponašanja steganografskih tehnika pri radu s kompresiranim (.jpg) i nekompresiranim (.pdf) formatima.
<p align="center">
<img width="941" height="586" alt="image" src="https://github.com/user-attachments/assets/3b9574be-2047-466f-bd9f-ba55e768b128" /></p>
<p align="center"><em>Slika 13: Originalna slika u .jpg formatu</em></p>

<p align="center">
<img width="939" height="422" alt="image" src="https://github.com/user-attachments/assets/b4ac3d0b-525b-499d-b1ef-b3843361a69a" /></p>
<p align="center"><em>Slika 14: Originalna slika u .png formatu</em></p>

Slika 1 (.jpg) imala je početnu veličinu 633,5 kB (633.463 bytes). Budući da Stegano omogućuje manipulaciju slikovnih datoteka putem terminala, alat je instaliran sljedećim naredbama:
...
Nakon instalacije, u .jpg sliku umetnuta je jednostavna tekstualna poruka korištenjem LSB (*Least Significant Bit*) metode. U tu svrhu poslužila je naredba:
...
# 5. Zlouporaba SMTP-a - Dino Primorac

## 5.1. Teorijski dio

### 5.1.1. SMTP
SMTP (Simple Mail Transfer Protocol) je temeljni internetski protokol za prijenos (transport) e-pošte između poslužitelja, zamišljen kao “store-and-forward” mehanizam u kojem se poruka postupno predaje s jednog mail poslužitelja na drugi dok ne dođe do odredišta. U klasičnom tijeku komunikacije SMTP klijent se spaja na SMTP poslužitelj, prima početni pozdrav (tipično odgovor 220), zatim se identificira naredbom EHLO/HELO, nakon čega slijedi definiranje “omotnice” poruke naredbama MAIL FROM (adresa pošiljatelja u omotnici) i RCPT TO (jedan ili više primatelja), a tek onda dolazi prijenos sadržaja poruke naredbom DATA, gdje se šalju zaglavlja i tijelo poruke te se prijenos završava posebnom oznakom kraja sadržaja. Taj je tok i logika protokola standardizirana i detaljno opisana u specifikaciji SMTP-a, uključujući uloge klijenta i poslužitelja te standardne kodove odgovora. (Klensin, 2008). U praksi, SMTP se najčešće koristi na portu 25 za komunikaciju između poslužitelja (MTA-ova), dok se slanje iz korisničkih aplikacija (mail klijenata) često odvaja u zaseban “submission” način rada, koji je standardno vezan uz port 587 i dodatna pravila oko autentikacije i politike slanja. (Gellens & Klensin, 2011).

### 5.2. SMTP Server
Kad se govori o SMTP serveru, najčešće se misli na poslužiteljsku komponentu koja prihvaća SMTP veze i obavlja ulogu MTA (Mail Transfer Agent) i/ili MSA (Mail Submission Agent), ovisno o kontekstu. Kao MTA, server prima poruke od drugih poslužitelja i prosljeđuje ih dalje ili ih isporučuje u lokalni sustav, a pri tome smije mijenjati poruku uglavnom samo na način da dodaje tragove prijenosa (trace) poput “Received” linija. (Klensin, 2008). Kao MSA, server prima poruke od korisničkih aplikacija ili webmail sustava i primjenjuje stroža pravila koja pomažu spriječiti zloupotrebe poput “open relay” scenarija, tipično zahtijevajući autentikaciju pošiljatelja i provjeru sintakse prije nego što poruka uopće uđe u transportni sustav. (Gellens & Klensin, 2011). U modernim implementacijama vrlo je važna i zaštita prijenosa, pa se TLS koristi ili kroz STARTTLS “nadogradnju” postojeće veze ili kroz “implicit TLS” modele; STARTTLS kao SMTP proširenje definira kako se SMTP sesija može prebaciti u šifrirani kanal, a novije smjernice naglašavaju da je slanje i pristup mailu bez TLS-a zastarjelo za scenarije između korisničkog klijenta i submission/access poslužitelja. (Hoffman, 2002; Moore & Newman, 2018).

### 5.3. Zaglavlja i njihova zloupotreba
Zlouporaba zaglavlja u kontekstu ekfiltracije poruka (odnosno izvlačenja sadržaja izvan inormacijskog sustava) obično se ne svodi na to da napadač “hakira” zaglavlja u letu između legitimnih MTA-ova, nego na to da iskoristi činjenicu da su zaglavlja ulazni tekst koji aplikacije ponekad pogrešno sklapaju iz korisničkih unosa. Jedan čest obrazac je tzv. SMTP/email header injection, gdje aplikacija (npr. kontakt forma ili interni sustav za slanje obavijesti) ubaci nesanitarizirani korisnički unos u zaglavlje, pa napadač umetne prijelom retka (CRLF) i tako “dodaje” nova zaglavlja poput Bcc ili mijenja Reply-To, što može rezultirati time da se kopija poruke pošalje trećoj strani ili da se osjetljivi tokeni i sadržaj preslika izvan namijenjenog kanala. (PortSwigger, n.d.; Invicti, n.d.). Tehnika je konceptualno povezana s CRLF injection klasom ranjivosti, gdje umetanje CR i LF znakova omogućuje neočekivano proširivanje linija koje sustav interpretira kao odvojena zaglavlja ili naredbe. (OWASP, n.d.). U ekfiltracijskom smislu, problem je što “legitimna” aplikacija često šalje poruke koje sadrže povjerljive podatke (npr. linkove za reset lozinke, jednokratne kodove ili interne izvještaje), pa napadač ne mora provaljivati u mailbox da bi došao do sadržaja, nego pokušava natjerati sustav da sam pošalje kopiju na adresu pod napadačevom kontrolom. (PortSwigger, n.d.).

Drugi, suptilniji sloj zlouporabe jest korištenje zaglavlja kao mjesta za skrivanje ili prenošenje dodatnih podataka, jer standard formata poruke dopušta velik broj zaglavlja, uključujući i proširiva ili nestandardna polja, pa obrambeni sustavi ponekad više pažnje posvete tijelu poruke i privicima nego “rubnim” zaglavljima. (Resnick, 2008). Obrana se tu tipično oslanja na kombinaciju ispravnog programiranja (strogo filtriranje ulaza koji se stavlja u zaglavlja i zabrana CR/LF znakova), te na autentikacijske i integritetne mehanizme mail ekosustava poput SPF-a (provjera tko smije slati za domenu u smislu omotnice), DKIM-a (kriptografski potpis odabranih dijelova poruke uključujući zaglavlja) i DMARC-a (politike i “alignment” s vidljivim From poljem). (Kitterman, 2014; Crocker, Hansen, & Kucherawy, 2011; Kucherawy & Zwicky, 2015). U praksi, ekfiltracija e-poštom i dalje ostaje važan “vektor” jer je e-pošta tipičan kanal “data in motion”, pa se organizacije oslanjaju na DLP programe i kontrole koje nadziru i ograničavaju odljev osjetljivih informacija upravo kroz kanale poput e-pošte. (Liu & Kuhn, 2010).

Na kraju, korisno je zapamtiti dvije provjerene forenzičke heuristike: prvo, da su “Received” tragovi pouzdaniji pri vrhu lanca jer ih dodaju poslužitelji u tranzitu (dok se lažni “Received” može pojaviti niže kao dio originalnog sadržaja), i drugo, da razlika između “From:” (zaglavlje) i MAIL FROM/Return-Path (omotnica i trace) često otkriva spoofing ili neusklađenost politike slanja. (Klensin, 2008; Microsoft, n.d.).
